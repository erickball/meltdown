"""
Build (u, v) grid - Version 10.

Fixes:
1. Filter out problematic saturation point at index 274 (IAPWS discontinuity)
2. Use log(v) offsets that increase linearly with u

Compressed liquid: 8 curves with log10(v_offset) increasing linearly with u,
such that log10(offset) is 30% larger at u=800 compared to u=0.
"""
...
print(f"u_f range: {sat_u_f[0]:.2f} to {sat_u_f[-1]:.2f}")
print(f"v_f range: {sat_v_f[0]:.6f} to {sat_v_f[-1]:.6f}")

# Check that u_f is now monotonic
u_f_diffs = np.diff(sat_u_f)
if np.all(u_f_diffs > 0):
    print("u_f is monotonically increasing (good)")
else:
    bad_idx = np.where(u_f_diffs <= 0)[0]
    print(f"WARNING: u_f not monotonic at indices: {bad_idx}")
...
# Compressed liquid: 8 curves with LOG(v) offset increasing linearly with u
print("\nCompressed liquid targets:")

# Base log10(v_offset) values at u=0
# These correspond to v offsets of: 1e-7, 2e-7, 5e-7, 1e-6, 2e-6, 5e-6, 1e-5, 2e-5
base_log_offsets = np.array([-7, -6.7, -6.3, -6, -5.7, -5.3, -5, -4.7])

# We want log10(offset) to increase by 30% from u=0 to u=800
# log10(1.3) ≈ 0.114, so we add 0.114 to the log offset over 800 u
# At u: log_offset = base_log_offset + 0.114 * (u / 800)
# This means the actual offset multiplies by 1.3 for every 800 kJ/kg increase in u

def get_log_v_offset(base_log_offset, u):
    """Get scaled log10(v_offset) at given u value."""
    # Add 0.114 (log10(1.3)) per 800 kJ/kg of u
    return base_log_offset + 0.114 * (u / 800)

# u values: consistent 2 kJ/kg spacing throughout
u_values = np.arange(2, 1980, 2)  # Start at 2 to avoid triple point issues
...
for u in u_values:
    ...
    for base_log_off in base_log_offsets:
        ...
        dv = 10 ** log_off  # Convert back to linear v offset
        v = v_f - dv
...
# Show how offsets scale
print("\n  Offset scaling check:")
print("  u (kJ/kg) | base offset | scaled offset | ratio")
for u_check in [0, 400, 800, 1200, 1600]:
    base = 1e-7
    log_base = -7
    log_scaled = get_log_v_offset(log_base, u_check)
    scaled = 10 ** log_scaled
    print(f"  {u_check:9} | {base:.2e}   | {scaled:.2e}    | {scaled/base:.2f}x")
...
# Save
output = {
    'description': 'Water properties on (u, v) grid - v10 with log offset scaling',
    'n_points': len(results),
    'compressed_liquid_info': {
        'base_log_offsets': base_log_offsets.tolist(),
        'scaling': 'log10(offset) = base + 0.114 * (u / 800), i.e., 1.3x per 800 kJ/kg',
        'u_spacing': 2,
    },
...
output_path = Path(__file__).parent / 'uv_grid_data_v10.json'
