"""
Build (u, v) grid - Version 8.

- Compressed liquid: use v6 approach (v offsets from v_f at given u)
- Vapor: properly check u > u_g at given v
- Supercritical: more careful region definition
"""
...
# Build interpolator for vapor line: u_g(v)
# v_g decreases with T, so sort by v_g ascending
...
def is_valid_vapor(u, v):
    """Check if (u, v) is valid superheated vapor (u > u_g at this v)."""
    if v < v_crit:
        return False  # Too small v for vapor
    u_g_at_v = u_g_from_v(v)
    return u > u_g_at_v + 5  # Add small margin
...
# Compressed liquid (same as v6 - v offset from saturation at given u)
...
# Superheated vapor - use (u, v) grid with validity check
...
for u in np.arange(2000, 2800, 15):
    for v in np.logspace(np.log10(0.002), np.log10(v_crit), 20):
        # This region is tricky - skip clearly invalid states
        if u < u_crit and v < v_crit * 0.8:
            continue  # Probably compressed liquid
...
# Save
output = {
    'description': 'Water properties on (u, v) grid - v8',
    'n_points': len(results),
    'points': results,
}

output_path = Path(__file__).parent / 'uv_grid_data_v8.json'
