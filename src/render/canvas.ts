...
import { renderComponent, getTimeSeed, formatCorePowerLabel, worldToScreen, renderFlowConnectionArrows, renderPressureGauge, renderThermometers, ConnectionScreenEndpoints, renderBurstOverlays, renderBreakConnections, renderBuildingFloor, renderBuildingFrontEdge, projectCircleToEllipse, flowConnectionIdForPlantConnection, openingArrowEndpoints } from './components';
...
export class PlantCanvas {
  ...
  private perspectiveRuns: Array<{ conn: Connection; pts: Point[] }> = [];
  ...
  /** The 2.5D view's drawn connection curve under a screen point (nearest wins). */
  private perspectiveRunAt(screenPos: Point): Connection | null {
    let best: Connection | null = null;
    let bestD = 6; // px - a line is 4 px wide; give the pointer a little room
    for (const run of this.perspectiveRuns) {
      const d = distanceToPolylinePx(screenPos, run.pts);
      if (d <= bestD) { bestD = d; best = run.conn; }
    }
    return best;
  }
