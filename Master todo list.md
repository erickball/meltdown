...
Playtest batch (2026-07-13, from Erick) - REMAINING:
-Some pre-made preset connections are direct even though they're long/wide enough to need a pipe (18 offenders across pwr/two-loop/level1 JSONs). Deferred: baking pipe nodes into converged preset loops changes their physics (extra nodes in every primary leg) and risks the tuned presets; needs a decision - bake pipes in and re-converge, or exempt presets from the rule.
-Deploy the updated Firebase functions (jackChat now declares file_car; new fileCar endpoint writes to the jack-cars Firestore collection) - until deployed, Jack's CAR reports park in localStorage 'meltdown_pending_cars'.
