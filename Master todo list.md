...
-Meltdown remaining: molten fuel relocation/corium (slumping to the lower head, geometry change, molten pool heat transfer to the vessel wall - pairs with vessel creep rupture); aerosol deposition/plate-out (CsI currently stays airborne forever); decay heat should follow released/relocated fission products; UI for melt fraction and radiological release (sim + headless logger show them; environmentalRelease Xe/CsI is the source term for the game-mode MACCS-lite).
