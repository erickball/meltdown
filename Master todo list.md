## TODO List
AI:
STATUS: SHIPPED (see docs/atom-jack.md). "Atom" Jack is live on hosting, backed by the jackChat Cloud Function (project unityriskresearch). Model upgraded from claude-sonnet-5 to claude-opus-5 (2026-07-26); monthly spend cap raised $10 -> $25. Character prompt, tools, and $/mo ledger all done. Remaining/known gaps below.
-AI assistant to help users add to, fix, or understand the model (uses opus 5, cap usage at $25/month. context: a bunch of game documentation, the current plant model - full list of components and their connections and properties, the last few changes the user made, what mode they're in, and the most recent simulation results for this model (if any). Also the ability to look at the code but let's try to make the documentation good enough it doesn't usually have to.) [DONE]
-and Opus plays the character of "Atom" Jack, head of operations at your primary EPC contractor Atom Enterprises. Jack knows this is all digital but doesn't bring it up unless the user does first. He has a colorful irreverent personality and likes to refer to himself as "just the plumber" and similar, although he does understand the engineering pretty well. He can help with creating customized components (e.g. "add a relief valve to this tank" and he can come up with reasonable sizing and setpoints and add the component to the model). [DONE - character prompt written, in-corner UI with expanding chat + scripted intro shipped]
-Firebase has my API key. [DONE - ANTHROPIC_API_KEY in unityriskresearch Secret Manager]
- Follow-up: tune Jack's default context so he has the program/instantiation background he needs to be effective at typical tasks (component creation/connection, diagnosing plant behavior). See test findings from 2026-07-26 context review.
...
-Advanced reactors remaining: metal coolant (sodium/lead - needs a second liquid property system, big); direct Brayton cycle (turbine operator assumes steam - gas loops must use an HX + steam secondary for now).
-TRISO barrier model (agreed design, unbuilt): (1) SiC retention factor on fission-product release - the CORSOR path is LWR-calibrated and gives TRISO no barrier credit, so pebble cores currently over-release regardless of oxidation; this is the biggest fidelity gap and is independent of burning. (2) OPyC as an oxidizable inventory on the existing series-resistance chain with a dense-PyC grade (low porosity/BET). (3) Exposed-particle fraction from burn-off weighted by the Thiele eta (eta<<1: only surface pebbles lose their shells; eta~1: oxidant reaches every particle already). (4) SiC oxidation LAST: parabolic passive (SiO2 protective layer, Baker-Just shape) smoothly blended via the Wagner criterion to linear active oxidation (SiO gas) at low pO2 + high T - note the air-ingress fire test already produces exactly that oxygen-starved-and-hot corner deep in the bed. (5) Release gated on exposed AND (SiC breached OR as-manufactured defective, ~1e-5..1e-4), so the defect fraction dominates mild cases - matching the real safety case where denuded/defective particles, not SiC consumption, drive release.
-Heat exchanger shells are never creep-checked: createWallThermalNodes skips heatExchanger, so the SG shell has no wall thermal node and only gets pressure-burst checks. Fine for sane designs (the well-mixed shell node sits at outlet temperature, ~300 C, matching the real HTGR practice of routing only cold gas along the pressure shell), but a plant that soaks a shell in 700 C gas would never creep-rupture. Fix: give HX components a shell wall node fed by shell-side gas temperature so the existing Larson-Miller machinery covers them. Related nit: the Xe-100 coaxial annulus sits upstream of the circulator (SG -> annulus -> pump) where the real plant puts it downstream (pump -> annulus -> RPV) - only a ~1 bar difference on the annulus, but flipping the generator plumbing would be more faithful.
-Graphite oxidation follow-ups (the operator itself is done - GraphiteOxidationRateOperator, one series-resistance chain for C+O2/H2O/CO2): (a) burn-off does not yet feed back into anything. Pebble burn-off should expose TRISO kernels and drive a fission-product release, and lost graphite should shrink solidModeratorVolume so moderator burn-off shows up as reactivity (the Windscale effect). (b) A3-3 pebble-matrix oxidation data is a stand-in - IG-110's numbers are used, flagged in graphite.ts. Matrix graphite is generally MORE reactive than reflector grades, so pebble burn rate is likely understated. (c) Steam/CO2 pre-exponentials are anchored on the classic ~1e5:3:1 O2:H2O:CO2 reactivity ratio rather than measured for these grades; activation energies are real. (d) The reflector radiates straight to the vessel wall, ignoring the core barrel in between, which physically acts as a radiation shield and would roughly halve the exchange - harmless today because that link has 10-100x the capacity it needs.
-Refit Zr-steam cladding oxidation (CladdingOxidationRateOperator, src/simulation/operators/rate-operators.ts) onto a mechanistic, threshold-free formulation. It currently violates the no-special-cases rule in five places: a hard T_THRESHOLD skip, steamFactor = min(1, P_steam/1e5) with a `< 0.01 continue` cutoff, an arbitrary 0.01 multiplier for steam over a liquid surface, Math.max(W, 1e-6) to dodge the parabolic singularity at W=0, and dW_dt capped at 1e-3 m/s. The fix: keep Baker-Just for the solid-state oxide-layer diffusion (that part is real physics, unlike the porous-gas transport in graphite), but replace the steam-availability fudge with the same external-mass-transfer series resistance used by the graphite operator, so steam depletion falls out of a linear C_bulk term instead of a clamped factor. The W->0 singularity wants a native-oxide initial thickness (~1-5 nm, physically real) instead of the 1e-6 guard, which also removes the need for the rate cap.
...
## Done List
X Pump/valve pressure rating dialog fields (2026-07-17): all valve dialogs (gate/globe/ball/butterfly, check, relief, PORV), the pump dialog, and the TD pump dialog now have an explicit Pressure Rating field (bar) that flows to the component (silencing the factory's service-derived-rating warning) and into cost: pump/valve cost gets a pressure-class multiplier 0.5 + 0.5*(rating/150 bar), so a 600-bar valve costs ~2.5x a 150-bar one and low-pressure bodies keep a machining floor. Defaults: valves 155, relief/PORV 200 (above their setpoints), pumps 150. Condenser's auto-created condensate pump ships rated 25 bar. Edit dialogs prefill and write back through the existing generic pressureRating path.
X Two-loop ~103% overshoot fixed (2026-07-17): measured why - the T-cold rod scheme can't do better on its own: T_cold moves only ~0.09 K per % power here, so the controller's 1 K deadband spans ~11% power (lowering the setpoint to 558.7 K parked the plant at 86.7%), and the 102.8% figure is coast-past: withdrawal stops at the permissive but the rod position reached corresponds to a ~103% equilibrium after temperature feedback settles. Fix: keep the always-withdraw 567 K setpoint and set the preset's withdrawal permissive (powerLimit) to 0.97 so the coast lands at rated - converges to 99.5% with reactivity ~0 at 900 s, startup ramp unchanged, Level 4 test PASSES. (A general fix for all presets would be overpower rod-runback - insertion above the power limit - in the shared rod controller; that changes every preset's convergence and needs Erick's call. Would also address HTGR's 109%.)
X Pebble-bed cores render as pebbles (2026-07-17): reactor-vessel graphic draws fuelForm 'pebbles' core barrels as a hex-packed bed of temperature-colored spheres with dark graphite rims and deterministic per-pebble jitter (no frame shimmer), floored at ~5 px and thinned above ~1200 drawn pebbles so any zoom stays cheap; control rods draw at the core periphery (side-reflector channels) instead of hanging in the bed. Rod-lattice cores unchanged.
X No more unbreakable components (2026-07-14): the burst initializer used to silently SKIP any component without a pressureRating (preset pumps, valves, turbines, and the auto-created turbine exhaust pipe had none - they could never burst). Now: shipped presets/levels carry explicit ratings (scripts/add-ratings.cjs: 1.5x SERVICE pressure - initial + pump rated head, max over connected neighbors, so FW pumps filled with 0.05-bar condensate still rate for their ~65-bar discharge and accumulator check valves rate for full RCS); anything still unrated at sim build gets the same service-derived rating plus a loud [Factory] UNBREAKABLE warning; the auto-created turbine exhaust pipe is rated like its turbine (1.5x inlet). Flow-physics test rigs got explicit 500-bar ratings (the deadhead test's dead-end tank started bursting mid-test - proof the change works). All presets load warning-free and behave; 25+10 tests pass.
