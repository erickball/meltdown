...
export const JACK_TOOLS: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    name: "file_car",
    description:
      "File a Corrective Action Report (CAR) - a bug report for the game's developers. Use when the user reports (or you observe) behavior that looks like a defect in the game itself: physics that can't be right, UI that misbehaves, tools that error on valid input, numbers that contradict each other. Not for player mistakes or design questions. Summarize the problem factually; game context (mode, sim time, selection, build) is attached automatically, and once a simulation has been built the report can also carry a reproduction bundle: the plant design, the live simulation state and the rewind history, so the developers can replay exactly what happened - you do not need to describe the plant in detail when that is attached. The user is shown the full report and must approve it before anything is sent, and they may withhold the context and/or the bundle - so write the description as something they will read. (A user who picked \"don't ask again\" is not prompted; the report goes straight out and they get a notice.) Check the tool result before you speak: if it says the user declined, the report was NOT filed, so don't claim otherwise and don't refile unless asked.",
    input_schema: {
      type: "object",
      properties: {
        title: {
